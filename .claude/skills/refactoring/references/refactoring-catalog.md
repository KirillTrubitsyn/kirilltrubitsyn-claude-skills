...
- **IDE refactoring** (при локальной работе): VS Code / WebStorm имеют встроенные рефакторинги (rename, extract function). Для TS они безопасные и учитывают все использования — пользуйся. В облачной/агентной сессии эквивалент — точечные правки с прогоном `tsc --noEmit` и тестов после каждого шага; массовые переименования — через `ts-morph` (он тоже учитывает typechecker; до стабилизации программного API в TS 7.1 работает поверх TS 6).
