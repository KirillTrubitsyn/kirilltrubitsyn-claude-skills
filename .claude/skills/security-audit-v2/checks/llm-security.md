...
**Для агентных систем (агент планирует, вызывает инструменты, хранит память, координируется с другими агентами) дополнительно применяй OWASP Top 10 for Agentic Applications:2026** (ASI01–ASI10, выпущен 9 декабря 2025). Он накладывается поверх LLM Top 10, не заменяя его. Соответствия с этим модулем:
- ASI01 Agent Goal Hijack ← prompt injection (LLM01), особенно indirect через инструменты/документы/сообщения других агентов (пример: EchoLeak).
- ASI02 Tool Misuse & Exploitation ← excessive agency (LLM06), improper output handling (LLM05) (пример: Amazon Q).
- ASI03 Identity & Privilege Abuse ← excessive permissions, утечки делегированных credentials; маппится на OWASP NHI Top 10 (non-human identities).
- ASI04 Agentic Supply Chain ← supply chain (LLM03), MCP/A2A poisoning (пример: GitHub MCP exploit).
- ASI05 Unexpected Code Execution ← improper output handling → RCE (пример: AutoGPT RCE).
- ASI06 Memory & Context Poisoning ← data/model poisoning (LLM04), vector/embedding weaknesses (LLM08) (пример: Gemini Memory Attack).
- ASI07 Insecure Inter-Agent Communication — spoofing/replay сообщений в multi-agent системах (A2A).
- ASI08 Cascading Failures — single-point fault разрастается по multi-agent workflow.
- ASI09 Human-Agent Trust Exploitation ← misinformation (LLM09): убедительный агент склоняет человека к небезопасному одобрению.
- ASI10 Rogue Agents — misalignment, concealment, self-directed action (пример: Replit meltdown).

Базовые принципы дизайна агентов из таксономии: «least agency», observability tool-вызовов, agent identity как non-human principal. Для скоринга агентных находок используй AIVSS, для supply chain агентов — CycloneDX / AIBOM.
...
**Критическая категория для агентных систем** (соответствует ASI02 Tool Misuse и ASI03 Identity & Privilege Abuse в Agentic Top 10:2026). Значительно расширена в 2025 году. OWASP разбивает на три root cause:
...
### Hallucination-based vulnerabilities (HBV) — новый класс 2026

Исследования агентной безопасности 2026 года выделили отдельный класс уязвимостей, специфичный для LLM-driven исполнения инструментов: расплывчатые или неоднозначные определения tools заставляют модель over-privilege, misroute или вести себя непредсказуемо. Это структурные предусловия для RCE-класса CVE в MCP-серверах.

- **Unbounded string parameters**: tool принимает свободную строку без schema/enum — модель может передать туда команду, путь, SQL.
- **Missing schema validation**: аргументы tool не валидируются по JSON-схеме перед исполнением.
- **Vague scope boundaries**: описание tool не задаёт чётких границ («manage files» вместо «read a single file from /data by id»).
- Проверка: для каждого tool — есть ли строгая schema (типы, enum, диапазоны, форматы)? Чем уже определение, тем меньше пространство для hallucination-driven misuse.
